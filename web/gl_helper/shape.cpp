...
/*** Function ***/
Object::Object(GLuint position_loc, GLuint color_loc,
               const std::vector<Object::Vertex> &vertex_list,
               const std::vector<GLuint> &index_list) {
  glGenVertexArrays(1, &m_vao);
  glBindVertexArray(m_vao);

  glGenBuffers(1, &m_vbo);
  glBindBuffer(GL_ARRAY_BUFFER, m_vbo);
  glBufferData(GL_ARRAY_BUFFER, vertex_list.size() * sizeof(Vertex),
               vertex_list.data(), GL_STATIC_DRAW);
  glVertexAttribPointer(position_loc, 3, GL_FLOAT, GL_FALSE, sizeof(Vertex),
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, index_list.size() * sizeof(GLuint),
...
void Object::Bind() const { glBindVertexArray(m_vao); }

Shape::Shape(const std::vector<Object::Vertex> &vertex_list,
             const std::vector<GLuint> &index_list) {
  /* Load Shader Program */
  static const GLchar vsrc[] =
      ...
      "}";
  static constexpr GLchar fsrc[] = "#version 300 es\n"
                                   "precision mediump float;\n"
  ...
  m_modelviewprojection_loc =
      glGetUniformLocation(m_program_id, "modelviewprojection");

  m_object = std::make_unique<Object>(position_loc, color_loc, vertex_list,
                                      index_list);
...
void Shape::Draw(const Matrix &viewprojection, const Matrix &model) const {
  ...
  glUniformMatrix4fv(m_modelviewprojection_loc, 1, GL_TRUE,
...
void Shape::SetLineWidth(float width) { glLineWidth(width); }

ShapeIndex::ShapeIndex(const std::vector<Object::Vertex> &vertex_list,
                       const std::vector<GLuint> &index_list)
...
ShapeSolidIndex::ShapeSolidIndex(const std::vector<Object::Vertex> &vertex_list,
                                 const std::vector<GLuint> &index_list)
    : Shape(vertex_list, index_list) {
  // do nothing
}
